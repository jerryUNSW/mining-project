    ...
    for i in range(len(suc_press_list)):
        ...
        if pd.isna(suc_val):
            ...
            if i+1 < len(P0N_final):
                ## instead of getting the adjacent cell, get the next not-null value! 
                curr = i+1
...
def compute_resistance(P0N_final, alr_list, alr_list_dropna, P01_EQ_list, q1_eq_list, P02_EQ_list, q2_eq_list):
    ...
    if num_of_holes == 2:
        ...
        if P02 - P01 >0:
            ...
            R02 = P02 / Q2 
        res_list = [R01, R02]
        res_list = [0 if pd.isnull(x) else x for x in res_list]
        res = P0N_final.copy()
        for i in range(len(P0N_final)):
            if not pd.isnull(P0N_final[i]):
                res[i] = res_list.pop()
    ...
    if num_of_holes == 3:
        ...
        if P03 - P02 > 0 and P02 - P01 >0 :
            ...
            to_pop = [R01, R02, R03]
            to_pop = [0 if pd.isnull(x) else x for x in to_pop]
            # print(R01_EQ, R02)
            res = P0N_final.copy()
...
for i in range(len(df1)):
    ...
    resistance_list = compute_resistance(P0N_final, alr_list, alr_list_dropna, 
        P01_EQ_list, q1_eq_list, P02_EQ_list, q2_eq_list)
    ...
    if resistance_list is not None:
        ...
        resistance.append(P0N_final)
        # print(num_itr, "\n", P0N_final)
        # print("")

df = pd.DataFrame(resistance[0:], columns=df1.columns[1:])

df.to_excel("resistance_approach1.xlsx")  
